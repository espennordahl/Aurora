...
int main (int argc, const char * argv[])
{
	...
	Camera cam = Camera(M_PI/8.75, 512, 512, 256, cameraSampler);
    ...
    objTrans.translate(Vector(0, 0, 0));
        //	objTrans.translate(Vector(0, scale/2, 0));
	objTrans.scale(scale, scale, scale);
	objTrans.translate(Vector(-300.f/scale, 0.f, -230.f/scale));
	objTrans.translate(Vector(278.f/scale, -273.f/scale, -800.f/scale ));
	Transform objTransInv = objTrans.inverse(objTrans);
	
	Reference<Shape> shape = new ObjTriangleMesh(&objTrans, &objTransInv, "/Users/espennordahl/Downloads/bunny.obj");
	
        //	Reference<Material> cubeBSurf = new MatteMaterial(Color(0.8));	
	Reference<Material> cubeBSurf = new KelemenMaterial(Color(1.f), 50., 0.9);	
